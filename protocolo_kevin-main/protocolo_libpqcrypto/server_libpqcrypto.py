...
def desencapsulado(id):
    ...
    kem = pqcrypto.kem.kyber512

    # generamos la llave pública y secreta
    ...
    ciphertext = connection.recv(800)
    ...
    # declaramos la función hash y hasheamos el Ct con el ID
    hash = SHA256.new()
    ...
    plaintext_recovered = kem.dec(ciphertext, secret_key)
...
while True:
    ...
    try:
        ...
        public_key_sign, secret_key_sign = dilithium2.keypair()
        # Enviamos al Cliente la llave publica
        ...
        # libpqcrypto arrojando firmas de 2076 bytes de longitud
        signature = dilithium2.sign(mi, secret_key_sign)
        # Concatenamos el ID_C, el bloque de firma, el mensaje declarado anteriormente y el mensaje
        ...
        # print(signature)
        connection.send(signature[0:1038])
        # print(".")
        time.sleep(.5)
        # print(".")
        connection.send(signature[1038:2076])
