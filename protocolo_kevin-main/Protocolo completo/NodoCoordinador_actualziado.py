...
'''
class StatTracker:
    output_file_template = 'StatResults{0}.txt'
    estimated_cpu_cycles = 0

    def set_stats(self):
        # Se usa perf_counter al ser de precision, es el timer usado por timeit
        # No se usa timeit en esta clase dado que será una medición única y con tareas bloqueantes
        self.elapsed_time = time.perf_counter() - self.initialization_time
    ...
    def write_stats(self, stage):
        ...
        with open(self.output_file, 'a') as f:
            f.write("Uso de memoria {0}\n".format(stage))
            f.write(str(self.used_mem / 1024) + " MB\n")
            f.write("Tiempo transcurrido {0} (s)\n".format(stage))
            f.write(str(self.elapsed_time) + "\n")
            f.write("Ciclos de reloj estimados {0}\n".format(stage))
            f.write(str(self.estimated_cpu_cycles) + "\n")
    ...
    def __init__(self):
        ...
        self.output_file = self.output_file_template.format(now.strftime(f"%m %d %Y %H:%M:%S"))
        with open(self.output_file, 'w') as f:
            f.write("Estadísticas del {0}".format(now.strftime(f"%m %d %Y %H:%M:%S")) + "\n")
        self.write_stats("inicial")
...
try:
    tracker = StatTracker()
    # FASE 0
    tracker.write_stats("inicio fase 0")
    id_sensor = corre_imu(server_sock)
    print("El ID del sensor es:")
    print(id_sensor)

    # FASE1
    tracker.write_stats("inicio fase 1")
    print('connecting to {} port {}'.format(*server_address))
    sock.connect(server_address)
    ...
    # FASE2
    tracker.write_stats("inicio fase 2")
    ...
    # INICIOFASE3
    tracker.write_stats("inicio fase 3")
