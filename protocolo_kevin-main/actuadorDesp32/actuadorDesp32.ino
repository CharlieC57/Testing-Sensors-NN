...
const char* ssid = "IZZI-C235";
const char* password = "3C046104C235";
...
void set_hash(){
  ...
  if (server.method() != HTTP_POST) {
    ...
    mbedtls_md_update(&ctx, (const unsigned char *)IDD.c_str(), IDD.length());
    mbedtls_md_finish(&ctx, shaResult);
    mbedtls_md_free(&ctx);
    char msg_hash_array[32];
    //String stringOne = (char*)resultArray;
    String stringOne = String((char*)shaResult);
    String stringTwo = stringOne.substring(0,32);
    msg_hash.toCharArray(msg_hash_array, 32);
    if (memcmp(msg_hash_array,shaResult,32)) {
